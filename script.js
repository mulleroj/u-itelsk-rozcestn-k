document.addEventListener('DOMContentLoaded', () => {
    ...
    // Mathematical camera positioning function
    function getCameraCoords(nodePctX, nodePctY, targetPctX, targetPctY, scale) {
        const stageWidth = window.innerWidth * 2.2;
        ...
        mm.add("(min-width: 1025px)", () => {
            ...
            tl.to(".callout-intro", { autoAlpha: 0, pointerEvents: "none", duration: 1.0 })
              ...
              .to(".world-stage", {
                  ...
                  y: () => getCameraCoords(0.50, 0.70, 0.50, 0.65, 1.25).y,
                  scale: 1.25,
                  duration: 1.5
              }, "-=0.8")
              .to(".scene-crossroads", { autoAlpha: 1, scale: 1, y: 0, duration: 1.2 }, "-=1.2")
              // Parallax background drift
              .to(".scene-background", { x: () => window.innerWidth * 2.2 * 0.50 * -0.04, y: () => window.innerHeight * 1.5 * 0.70 * -0.04, duration: 1.5 }, "-=1.5");

            // Chapter 1 -> 2: AI World enters (Crossroads hide)
            tl.to(".scene-crossroads", { autoAlpha: 0, scale: 0.75, y: -50, duration: 1.0 })
              .to(".world-stage", {
                  x: () => getCameraCoords(0.27, 0.57, 0.65, 0.50, 1.50).x,
                  y: () => getCameraCoords(0.27, 0.57, 0.65, 0.50, 1.50).y,
                  scale: 1.50,
                  duration: 1.5
              }, "-=0.8")
